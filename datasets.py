...
# Class to return dataset object with applied transoformations for the train and val subsets
class TransformedDataset(Dataset):
    """
    Dataset class that applies transformations after 
    splitting the dataset into a train and val subset.
    """
...
class EvaluationDataset(Dataset):
    """
    Dataset used when running the evaluation script
    """
    ...
    def __getitem__(self, index):
        img_name = os.path.join(self.root, self.imglist[index])
        img = PIL.Image.open(img_name)
        if self.transform:
            img = self.transform(img)
        return img

def split_dataset(dataset, transform_train, transform_val):
    """
    Function that splits the ImageFolder dataset into a train and val subset.
    """
    # Create the index splits for training, validation and test
    num_train = len(dataset)
    targets = dataset.targets # extracts a list of all classes in the ImageFolder dataset
    #np.random.seed(1) # ensure consistent splits
    indices = list(range(num_train))
    
    # random state equals an integer to ensure the same split on every run
    train_idx, valid_idx = train_test_split(indices, test_size = 0.1, random_state = 42, shuffle = True, stratify = targets)

    #print('train idx', train_idx)
    #print('val idx', valid_idx)

    training = torch.utils.data.Subset(dataset, train_idx)
    validation = torch.utils.data.Subset(dataset, valid_idx)
    
    training_dataset = TransformedDataset(training, transform_train)
    validation_dataset = TransformedDataset(validation, transform_val)
    
    return training_dataset, validation_dataset

# MSLS dataset official implementation: https://github.com/mapillary/mapillary_sls/blob/master/mapillary_sls/datasets/msls.py

#  Copyright (c) Facebook, Inc. and its affiliates.

# Modified to allow packaging two different positives into the triplets. 
...
class MSLS(Dataset):
    def __init__(self, root_dir, cities = '', nNeg = 5, transform = None, mode = 'train', task = 'im2im', subtask = 'all', seq_length = 1, posDistThr = 10, negDistThr = 25, cached_queries = 1000, cached_negatives = 1000, positive_sampling = True, num_positives=1):
        ...
        for city in self.cities:
            ...
            if self.mode in ['train','val']:
                ...
                for q_seq_idx in range(len(qSeqKeys)):
                    ...
                    # the query image has at least two positives when using two outputs
                    if len(p_uniq_frame_idxs) >= (self.num_positives):
    ...
    def update_subcache(self, output_dim, net = None):
        ...
        # uncomment if using RandErasing
        #torch.multiprocessing.set_start_method('spawn')
        
        
        with torch.no_grad():
            
            # initialize descriptors - changed to output dim of model
            ...
            bs = opt['batch_size']
            # compute descriptors
            for i, batch in tqdm(enumerate(qloader), position=0, leave=True, desc = 'compute query descriptors'):
                X, y = batch
                qvecs[i*bs:(i+1)*bs, : ] = net(X.to(self.device)).data
            for i, batch in tqdm(enumerate(ploader), position=0, leave=True, desc = 'compute positive descriptors'):
                X, y = batch
                pvecs[i*bs:(i+1)*bs, :] = net(X.to(self.device)).data
            for i, batch in tqdm(enumerate(nloader), position=0, leave=True, desc = 'compute negative descriptors'):
                X, y = batch
                nvecs[i*bs:(i+1)*bs, :] = net(X.to(self.device)).data

        print('>> Searching for hard negatives...', flush = True)
        # compute dot product scores and ranks on GPU
        pScores = torch.mm(qvecs, pvecs.t())
        # returns sorted values and their indices in the original input
        ...
        for q in range(len(qidxs)):
            ...
            # TODO: fix so it does this for the second positive too
            ...
            cached_pidx.append(pRanks[q, pidx][0][0])
            if self.num_positives > 1:
                cached_pidx.append(pRanks[q, pidx][0][1])
            ...
            #target = [-1] + [1]*len(pidx)  + [0]*len(hardestNeg)
    ...
    def __getitem__(self, idx):
        ...
        if self.num_positives > 1:
            ...
            output.extend([torch.stack([self.transform(Image.open(im)) for im in self.dbImages[idx].split(',')]) for idx in pidx])
